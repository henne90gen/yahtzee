...
function DieSvg(props: { die: Die, disabled: boolean, onDieLockChange: () => void }) {
        ...
        <div className="grid grid-cols-5">
            {dice.map((d, i) => (
                <div key={i} className="px-2">
                    {d.value === 0 ? '?' : <DieSvg die={d} disabled={disabled} onDieLockChange={() => dispatch(onDieLockChange(i))} />}
                </div>
            ))}
        </div>
    );
...
function StrikeSelector(props: { currentPlayer: Player, disabled: boolean, selectedOption: (option: EndTurnOption, strike: boolean) => void }) {
    const { currentPlayer, disabled, selectedOption } = props;
...
function DiceOptions(props: { currentPlayer: Player, dice: Die[], disabled: boolean, selectedOption: (option: EndTurnOption, strike: boolean) => void }) {
    const { currentPlayer, dice, disabled, selectedOption } = props;
    // TODO render a placeholder if disabled is true
...
function PlayArea() {
    ...
    const rollResults = <>
        <div className="text-center">{t("playing_RollCount")} {rollCount}/3</div>
